...
function uploadComplete(fileObj, server_data) {
	try {
		...
		progress.ToggleCancel(false);
		server_data.sub(/\[(.*?)\]\[(.*?)\]\[(.*?)\]/, function(match){
		  	progress.SetThumbnail(match[1], match[3]);
			progress.SetBigImage(match[2]);
			progress.SetId(match[3]);
		});
...
function uploadComplete_music(fileObj, server_data) {
	try {
		var progress = new FileProgress(fileObj, this.customSettings.progressTarget);
		progress.SetComplete();
		progress.SetStatus("Complete.");
		progress.ToggleCancel(false);
		server_data.sub(/\[(.*?)\]\[(.*?)\]\[(.*?)\]/, function(match){
		  	progress.SetMusicFile(match[3], match[1], match[2]);
			//progress.SetThumbnail(match[1], match[3]);
			//progress.SetBigImage(match[2]);
			//progress.SetId(match[3]);
		});
...
function FileProgress(fileObj, target_id) {
	...
	if (!this.fileProgressWrapper) {
		...
		$(target_id).insertBefore(this.fileProgressWrapper, $(target_id).firstChild);
		$(target_id).update($(target_id).innerHTML+'<script type="text/javascript">new Draggable("'+this.file_progress_id+'", { handle:\'drag_handle\', revert:true, reverteffect:function(element, top_offset, left_offset) { new Effect.MoveBy(element, -top_offset, -left_offset, {duration:0});}}) </script>');
	} else {
...
function editPhotoContainer(obj, method) {
	if (method == 'over') {
		var button = document.createElement('div');
		$(button).addClassName("photo-edit-button");
		$(obj).appendChild(button);
	}
	if (method == 'out') {
		$(obj).down('div.photo-edit-button').remove();
	}
} 
