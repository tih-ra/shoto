        ...
        begin
          ...
          when 20
            ...
            when 'play'
              ...
              on_play(@stream.duration)
            when 'getDuration'
              #@stream = @stream_pool.get(pkt.parsed_data.args[0])
              on_duration
              #IzumiLogger.debug "> INDEX:#{@stream}" 
            end
          else
...
private
    def handshake
      ...
      @sock.write( "\3" << HandshakeServer << Handshake.get_handshake(c_handcheck ))
    ...
    def on_duration
      @writer.send_duration
    end
    def on_createStream
      @writer.send_create_stream_result
    end
    
    def on_play(duration)
      ...
      @writer.send_play_start(duration)
